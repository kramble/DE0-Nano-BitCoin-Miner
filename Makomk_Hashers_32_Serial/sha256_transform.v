...
module sha256_transform #(
	...
	input fb_second,
	input [5:0] cnt,
	input [255:0] rx_state_1,
	input [511:0] rx_input_1,
	...
	generate
		always @ (posedge clk)
		begin
		  state_first <= feedback ? HASHERS[NUM_ROUNDS/LOOP-6'd1].state : (fb_second ? 256'h5be0cd191f83d9ab9b05688c510e527fa54ff53a3c6ef372bb67ae856a09e667 : rx_state_1);
			if(fb_second) begin
        rx_input_d[`IDX(0)] <= rx_state_1[`IDX(0)] + HASHERS[NUM_ROUNDS/LOOP-6'd1].state[`IDX(0)];
        rx_input_d[`IDX(1)] <= rx_state_1[`IDX(1)] + HASHERS[NUM_ROUNDS/LOOP-6'd1].state[`IDX(1)];
        rx_input_d[`IDX(2)] <= rx_state_1[`IDX(2)] + HASHERS[NUM_ROUNDS/LOOP-6'd1].state[`IDX(2)];
        rx_input_d[`IDX(3)] <= rx_state_1[`IDX(3)] + HASHERS[NUM_ROUNDS/LOOP-6'd1].state[`IDX(3)];
        rx_input_d[`IDX(4)] <= rx_state_1[`IDX(4)] + HASHERS[NUM_ROUNDS/LOOP-6'd1].state[`IDX(4)];
        rx_input_d[`IDX(5)] <= rx_state_1[`IDX(5)] + HASHERS[NUM_ROUNDS/LOOP-6'd1].state[`IDX(5)];
        rx_input_d[`IDX(6)] <= rx_state_1[`IDX(6)] + HASHERS[NUM_ROUNDS/LOOP-6'd1].state[`IDX(6)];
        rx_input_d[`IDX(7)] <= rx_state_1[`IDX(7)] + HASHERS[NUM_ROUNDS/LOOP-6'd1].state[`IDX(7)];
			  rx_input_d[511:256] <= 256'h0000010000000000000000000000000000000000000000000000000080000000;
			end else begin
			  rx_input_d <= rx_input_1;
			end
			if(feedback)
			  t1_part_first <= t1_part_fb;
			else if(fb_second)
           t1_part_first <= (32'h5be0cd19 + rx_state_1[`IDX(0)] + HASHERS[NUM_ROUNDS/LOOP-6'd1].state[`IDX(0)] + K_0);
			else
			  t1_part_first <= (rx_state_1[`IDX(7)] + rx_input_1[31:0] + K_0);
		end
		...
		for (i = 0; i < NUM_ROUNDS/LOOP; i = i + 1) begin : HASHERS
			...
			begin
				...
				if(LOOP == 1)
					...
					always @ (posedge clk)
						feedback_r <= HASHERS[i-1].feedback_r;
				...
				begin
					...
					if(LOOP == 1)
						assign cur_w0 = rx_input_d[31:0];
					else
						assign cur_w0 = feedback_r ? fb_w0 : rx_input_d[31:0];
				end
				else
				...
				begin
					...
					if(LOOP == 1)
						assign cur_w1 = rx_input_d[63:32];
					else
						assign cur_w1 = feedback_r ? fb_w1 : rx_input_d[63:32];
				end 
				else if(i < 8)
				begin
					...
					shifter_32b #(.LENGTH(i), .LEN_THRESH(6)) shift_w1 (clk, rx_input_d[`IDX(1+i)], nonfb_w1);
					if(LOOP == 1)
				...
				begin
					...
					if(LOOP == 1)
						assign cur_w14 = rx_input_d[479:448];
					else
						assign cur_w14 = feedback_r ? fb_w14 : rx_input_d[479:448];
				end
				else if(i == 1)
				begin
					wire [31:0] fb_w14; wire [31:0] nonfb_w14;
					shifter_32b #(.LENGTH(1)) shift_w14 (clk, rx_input_d[511:480], nonfb_w14);
					if(LOOP != 1)
				...
				begin
					...
					shifter_32b #(.LENGTH(i)) shift_w9 (clk, rx_input_d[`IDX(9+i)], nonfb_w9);
					if(LOOP == 1)
			...
			begin
				...
				begin
					reg [511:0] w_reg;
					always @ (posedge clk)
						w_reg <= feedback ? w_fb : rx_input_1; // FIXME - this is WRONG and BROKEN!
					assign cur_w = w_reg;
				end
				else
			...
			//assign w_fb = {HASHERS[NUM_ROUNDS/LOOP-6'd1].new_w15, HASHERS[NUM_ROUNDS/LOOP-6'd1].new_w14to0}; // FIXME
			assign t1_part_fb = HASHERS[NUM_ROUNDS/LOOP-6'd1].t1_part_next;
	endgenerate
