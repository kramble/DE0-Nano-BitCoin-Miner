/*
* HASHERS_3 for LX_9 based on hashers_11
*
* ---------------------- OLD ------------------
...
module sha256_transform #(
	// LOOP*NUM_HASHERS must be >= 64, and probably only works for 66
	// parameter LOOP = 22,
	// parameter NUM_HASHERS = 3
	parameter LOOP = 11,
	parameter NUM_HASHERS = 6	// Try hashers 6 (also change 21 to 10 below and in fpgaminer_top)
) (
	input clk,
	input phase_in,
	input [7:0]nonce_lsb_in,
	input [255:0] rx_state,		// This is always midstate, regardless of phase_in
	input [511:0] rx_input,
	output reg [255:0] tx_hash,
	output reg [6:0] tx_seq = 0,	// HASHERS_3 increased from [4:0] to [6:0]
	output reg [7:0] tx_nonce_lsb = 0,
	output reg tx_fullhash = 0	// NB hash is only valid for phase==0, hence tx_fullhash
);
	...
	wire [6:0] seq_fb;			// HASHERS_3 increased from [4:0] to [6:0]
	wire [7:0] nonce_lsb_fb;

	reg [255:0] state_first;
	reg [31:0] t1_part_first;
	reg [7:0] nonce_lsb_first;

	// HASHERS_3 needs 5 bits for k index rather than 3, so increase cnt reg to 7 bits
	// Due to the need to distinguish between phase 0, phase 1 and initial state, ~seq[6] indicates init
	// then seq[5] is phase, the lowest 5 bits are the k index (previously as cnt)
	// OLD..
	// Due to the need to distinguish between phase 0, phase 1 and initial state, ~seq[4] indicates init
	// then seq[3] is phase, the lowest 3 bits are the k index (previously as cnt)
	// This is a bit inefficient, but hopefully it will not matter and it does simplify the logic.
	reg [6:0] seq_fb_d = 0;			// HASHERS_3 increased from [4:0] to [6:0]
	wire feedback = (seq_fb_d[6] && seq_fb_d[4:0] != (LOOP-1));	// Now parameterised
	// wire feedback = (seq_fb_d[6] && seq_fb_d[4:0] != 5'd10);	// HASHERS_6 using 10 since LOOP=11
	// wire feedback = (seq_fb_d[6] && seq_fb_d[4:0] != 5'd21);	// HASHERS_3 using 21 since LOOP=22
	// wire feedback = (seq_fb_d[4] && seq_fb_d[2:0] != 3'd5);
	
	wire [31:0] K_0;
	assign K_0 = Ks[32*63 +: 32];
		
	wire [255:0] rx_state_mux = 
		phase_in ? 256'h5be0cd191f83d9ab9b05688c510e527fa54ff53a3c6ef372bb67ae856a09e667 : rx_state;
		
	generate
		always @ (posedge clk)
		begin
			state_first <= feedback ? state_fb : rx_state_mux;
			t1_part_first <= feedback ? t1_part_fb : (rx_state_mux[`IDX(7)] + rx_input[31:0] + K_0);
			nonce_lsb_first <= feedback ? nonce_lsb_fb : nonce_lsb_in;
			seq_fb_d <= seq_fb;
		end
		...
		for (i = 0; i < NUM_HASHERS; i = i + 1) begin : HASHERS
			...
			reg [6:0] cur_seq = 0;		// HASHERS_3 increased from [4:0] to [6:0]
			reg [7:0] cur_nonce_lsb;
							
			if(i == 0)
				always @ (posedge clk)
				begin
					// HASHERS_3 increased from [4:0] to [6:0]
					cur_seq <= feedback ? {seq_fb_d[6:5], seq_fb_d[4:0] + 5'd1} : { 1'b1, phase_in, 5'd0 };
					// OLD...
					// cur_seq <= feedback ? {seq_fb_d[4:3], seq_fb_d[2:0] + 3'd1} : { 1'b1, phase_in, 3'd0 };
					cur_nonce_lsb <= nonce_lsb_first;
				end
			else
				always @ (posedge clk)
				begin
					cur_seq <= HASHERS[i-1].cur_seq;
					cur_nonce_lsb <= HASHERS[i-1].cur_nonce_lsb;
				end
			...
			assign K_next = K_next_tbl[cur_seq[4:0]];	// LS bits of seq used to index K_next_tbl - HASHERS3 uses [4:0] cf [2:0]
		...
		begin
			...
			assign seq_fb = HASHERS[NUM_HASHERS-6'd1].cur_seq;
			assign nonce_lsb_fb = HASHERS[NUM_HASHERS-6'd1].cur_nonce_lsb;
		end
	endgenerate
		
	always @ (posedge clk)
	begin
		// For NUM_HASHERS=3, this still works (taking final output from stage 3-3 = 0) ...
		// We hardcode parameters LOOP=6 and NUM_HASHERS=11 giving 66 which is 2 in excess of
		// the 64 required, so need to take tx_hash output from the 20th stage.
		// NB rx_state is always midstate, so we do calculation of full hash based on const
		...
		tx_seq <= HASHERS[NUM_HASHERS-6'd3].cur_seq;
		tx_nonce_lsb <= HASHERS[NUM_HASHERS-6'd3].cur_nonce_lsb;
		// NB rx_state is always midstate, so we do calculation of full hash based on const
		// tx_fullhash <= HASHERS[NUM_HASHERS-6'd3].state[`IDX(7)] == (32'h0 - 32'h5be0cd19);
		tx_fullhash <= (HASHERS[NUM_HASHERS-6'd3].state[`IDX(7)] == 32'ha41f32e7);	// Explicit value of (32'h0 - 32'h5be0cd19)
	end
